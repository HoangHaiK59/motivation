...
const pinchEnd = proc((state, numberOfPointers) => Platform.OS === 'android' ? or(eq(state, State.END), lessThan(numberOfPointers, 2)) : eq(state, State.END));
const decay = (
    position,
    velocity,
    clock
...
export const usePinch = ({
    pinch, pan, translate, translationX, scale, minVec, maxVec
}) => {
